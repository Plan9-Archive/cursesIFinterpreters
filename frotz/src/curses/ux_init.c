...
*/
    ...
    f_setup.story_file = strdup((char*)(argv[optind]));
    f_setup.story_name = strdup((char*)(argv[optind]));

    /* Now strip off the extension. */
/*
    p = (char*)rindex(f_setup.story_name, '.');
*/
	p = (char*)(f_setup.story_name);
    if ((p != NULL) &&
        ((strcmp(p,EXT_BLORB2) == 0) ||
         (strcmp(p,EXT_BLORB3) == 0) ||
         (strcmp(p,EXT_BLORB4) == 0) ) )
    {
        blorb_ext = strdup((char*)p);
    }
    else
    {
        blorb_ext = strdup((char*)EXT_BLORB);
    }
    ...
    f_setup.story_path = strdup((char*)(argv[optind]));
